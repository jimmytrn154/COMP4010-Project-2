...
import numpy as np
import logging
from modeling.forecasting_utils import TASK_CONFIGS as FORECAST_TASK_CONFIGS, load_model_artifact, recursive_forecast
...
forecast_task_options = {
    key: cfg["title"] for key, cfg in FORECAST_TASK_CONFIGS.items()
}
forecast_artifacts = {
    key: load_model_artifact(key) for key in FORECAST_TASK_CONFIGS
}
...
@media (max-width: 1200px) {
    .overview-grid, .bottom-grid, .kpi-grid, .control-grid, .duo-grid, .method-grid, .forecast-filter-grid {
...
app_ui = ui.page_fluid(
    ...
    ui.div(
        ...
        # ===========================================================
        # ACT 5 — WHAT might happen next?
        # ===========================================================
        section_header(5, "Future risk forecast", "ML-based 1–12 month outlook by task"),
        ui.div(
            ui.div(
                ui.div("Forecast controls", class_="card-title"),
                ui.div(
                    "Choose a trained task model, set the forecast horizon, and focus the map and trend views on selected provinces.",
                ...
                ui.div(
                    ui.input_select(
                        "forecast_task",
                        "Prediction task",
                        choices=forecast_task_options,
                        selected="combined_risk",
                    ),
                    ui.input_slider("forecast_horizon", "Months ahead", 1, 12, 6),
                    ui.input_selectize(
                        "forecast_provinces",
                        "Provinces",
                        choices=provinces,
            ...
            ui.div(
                chart_card(
                    ...
                    "Color shows the average predicted value across the selected forecast horizon.",
                    "forecast_map_plot",
                    430,
                ),
                chart_card(
                    "Historical trend + forecast",
                    "Solid lines show history and dashed lines show the recursive forecast for selected provinces.",
                    "forecast_trend_plot",
                    430,
                ),
                class_="duo-grid",
            ...
            ui.div(
                chart_card(
                    "Top 5 forecast hotspots",
                    "Highest predicted province averages across the chosen forecast window.",
                    "forecast_top_plot",
                    300,
                ),
                ui.div(
                    ui.div("Forecast detail table", class_="card-title"),
                    ui.div(
                        "Province-month predictions for the selected task and horizon.",
                        class_="card-subtitle",
                    ),
                    ui.output_ui("forecast_table"),
                    class_="card",
                ),
                class_="duo-grid",
            ),
...
def server(input, output, session):
    ...
    @reactive.calc
    def selected_forecast_provinces():
        chosen = input.forecast_provinces()
        if chosen is None or chosen == "":
            return provinces
        if isinstance(chosen, str):
            return [chosen]
        return list(chosen) if len(chosen) else provinces

    @reactive.calc
    def current_forecast_artifact():
        return forecast_artifacts.get(input.forecast_task())

    @reactive.calc
    def forecast_results():
        artifact = current_forecast_artifact()
        if artifact is None:
            return pd.DataFrame(columns=["province_name", "forecast_date", "step", "predicted_value"])
        return recursive_forecast(
            model_panel_df,
            artifact,
            selected_forecast_provinces(),
            int(input.forecast_horizon()),
        )
    ...
    def forecast_status():
        ...
        if artifact is None:
            return ui.div(
                "No trained model artifact was found for this task yet. Run the notebook or training workflow first.",
                class_="warning-pill",
            )
        ...
        return ui.div(
            ui.div(
                f"Model: {artifact['best_model_name']} · Training coverage: {coverage.get('start', 'N/A')} to {coverage.get('end', 'N/A')}",
                class_="forecast-note",
            ),
            ui.div(
                f"Recursive one-step forecasting for {artifact['target_label']} across {len(selected_forecast_provinces())} selected provinces.",
                class_="forecast-pill",
            ),
        )

    @output
    @render_plotly
    def forecast_map_plot():
        artifact = current_forecast_artifact()
        fc = forecast_results()
        if artifact is None or fc.empty:
            return empty_fig(430, "No forecast available for this task")

        summary = (
            fc.groupby("province_name", as_index=False)["predicted_value"]
            .mean()
            .rename(columns={"predicted_value": "forecast_avg"})
        )
        fig = px.choropleth_mapbox(
            summary,
            geojson=geojson_data,
            ...
            color="forecast_avg",
            hover_name="province_name",
            hover_data={"forecast_avg": ":,.3f", "province_name": False},
    ...
    def forecast_trend_plot():
        ...
        fc = forecast_results()
        if artifact is None or fc.empty:
            return empty_fig(430, "No forecast trend available")

        target_col = artifact["target_col"]
        chosen = selected_forecast_provinces()
        history = (
            model_panel_df[
                model_panel_df["province_name"].isin(chosen) & model_panel_df[target_col].notna()
            ][["province_name", "date", target_col]]
            .sort_values(["province_name", "date"])
            .copy()
        )
        history = history.groupby("province_name").tail(36)

        fig = go.Figure()
        palette = px.colors.qualitative.Set2 + px.colors.qualitative.Bold
        color_map = {prov: palette[idx % len(palette)] for idx, prov in enumerate(chosen)}

        for province in chosen:
            hist = history[history["province_name"] == province]
            pred = fc[fc["province_name"] == province]
            if not hist.empty:
                fig.add_trace(
                    go.Scatter(
                        x=hist["date"],
                        y=hist[target_col],
                        mode="lines",
                        name=f"{province} history",
                        line=dict(color=color_map[province], width=2),
            ...
            if not pred.empty:
                fig.add_trace(
                    go.Scatter(
                        ...
                        line=dict(color=color_map[province], width=2, dash="dash"),
                        marker=dict(size=6),
                        legendgroup=province,
                    )
                )

        apply_dark_layout(fig, 430, legend=True)
        fig.update_yaxes(title=artifact["target_label"])
        fig.update_xaxes(title="")
        return fig

    @output
    @render_plotly
    def forecast_top_plot():
        artifact = current_forecast_artifact()
        fc = forecast_results()
        if artifact is None or fc.empty:
            return empty_fig(300, "No hotspot ranking available")
        ...
        summary = (
            ...
            .head(5)
            .sort_values("predicted_value", ascending=True)
        )
        fig = px.bar(
            summary,
            x="predicted_value",
            ...
            labels={"predicted_value": artifact["target_label"], "province_name": ""},
        )
        fig.update_traces(
    ...
    def forecast_table():
        ...
        fc = forecast_results()
        if artifact is None or fc.empty:
            return ui.div("No forecast table available.", class_="forecast-note")

        table_df = fc.copy()
        table_df["forecast_date"] = pd.to_datetime(table_df["forecast_date"]).dt.strftime("%Y-%m")
        table_df = table_df.rename(
            columns={
                "province_name": "Province",
                "forecast_date": "Forecast Month",
                "step": "Step",
                "predicted_value": artifact["target_label"],
            }
        )
        value_col = artifact["target_label"]
        table_df[value_col] = table_df[value_col].map(lambda v: f"{v:,.3f}")
        return ui.div(
            ui.HTML(table_df.to_html(index=False, classes="forecast-table", border=0)),
            class_="table-wrap",
        )
